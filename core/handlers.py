...
from services.auction import (
    auction_running, auction_update_task, auction_timers,
    get_auction_lots_for_display, place_bid,
    update_auction_lots, auction_update_loop,
    set_admin_auction_lots, refresh_auction_for_all,
    get_stars_display, get_stars_by_rarity,
    user_auction_page,
    frozen_bids
)
from config import (
...
# ========== ФУНКЦИЯ ДЛЯ ПОКАЗА ЛОТА АУКЦИОНА ==========
async def show_auction_lot(message, user_id: str, page: int):
...
def register_handlers(dp: Dispatcher):
    ...
    async def process_casino_bet(message: types.Message, state: FSMContext):
        ...
        try:
            ...
            logger.info(f"🎰 Ставка от {message.from_user.id}: {bet}")
            ...
            logger.info(f"💰 Баланс пользователя: {user['money']}")
            ...
            logger.info(f"✅ Ставка сохранена: {bet}")
    ...
    async def process_auction_bid(message: types.Message, state: FSMContext):
        ...
        if current_state != "waiting_for_auction_bid":
            # Если не в состоянии ставки - проверяем промокод
            await handle_promo(message, state)
            return
        ...
        try:
            ...
            logger.info(f"🚗 Ставка на аукцион от {user_id}: {amount}, страница: {page}")
    ...
    async def process_support_message(message: types.Message, state: FSMContext):
        ...
        try:
            ...
            logger.info(f"📩 Новое обращение от {user_id}: {message.text[:50]}...")
    ...
    async def business_menu(callback: types.CallbackQuery, state: FSMContext):
        ...
        try:
            ...
            logger.info(f"🏢 Открытие бизнеса для {user_id}")
    ...
    async def admin_help(message: types.Message):
        logger.info(f"👑 Команда /ahelp от {message.from_user.id}")
    ...
    async def stop_auction_lot(message: types.Message):
        ...
        if len(parts) < 2:
            await message.answer("❌ Использование: /stopauctionlot (номер лота)\n\nНомера лотов с 1 до 15")
            return
        
        try:
            lot_number = int(parts[1])
            
            if lot_number < 1 or lot_number > 15:
                await message.answer(f"❌ Номер лота должен быть от 1 до 15!")
                return
            
            data = await load_auction_data()
            lots = data.get("lots", [])
            
            if lot_number > len(lots):
                await message.answer(f"❌ Лот #{lot_number} не существует!")
                return
            ...
            if lot.get("sold", False):
                await message.answer(f"❌ Лот #{lot_number} уже продан!")
                return
            
            if not lot.get("is_active", True):
                await message.answer(f"❌ Лот #{lot_number} уже неактивен!")
                return
            
            # Возвращаем деньги текущему лидеру
            current_bidder = lot.get("current_bidder")
            current_bid = lot.get("current_bid", 0)
            
            if current_bidder and current_bid > 0:
                users = await load_users()
                if current_bidder in users:
                    frozen = frozen_bids.get(current_bidder, 0)
                    frozen_bids[current_bidder] = max(0, frozen - current_bid)
                    await save_users(users)
                    
                    try:
                        await bot.send_message(
                            int(current_bidder),
                            f"🔄 Лот #{lot_number} остановлен администратором!\n"
                            f"💰 Ваши {current_bid:,}₽ разморожены и доступны на балансе."
                        )
                    except Exception as e:
                        logger.warning(f"Не удалось уведомить пользователя {current_bidder}: {e}")
            ...
            car_name = lot.get("car_name", "Неизвестно")
            await message.answer(
                f"✅ Лот #{lot_number} ({car_name}) остановлен!\n"
                f"💰 Деньги возвращены владельцу ставки."
            )
            
        except ValueError:
            await message.answer("❌ Введите корректный номер лота!")
        except Exception as e:
            logger.error(f"Ошибка в stopauctionlot: {e}")
            await message.answer(f"❌ Ошибка: {e}")
    ...
    async def get_player_acc(message: types.Message):
        ...
        try:
            ...
            keyboard = [
                ...
                [InlineKeyboardButton(text="✅ Разблокировать", callback_data=f"admin_unban_{found_id}")],
    ...
    async def admin_unban(callback: types.CallbackQuery):
        ...
        user_id = callback.data.replace("admin_unban_", "")
        try:
            users = await load_users()
            if user_id in users:
                ...
                try:
                    await bot.send_message(int(user_id), "✅ Ваш аккаунт разблокирован администратором!")
                except:
                    pass
        except Exception as e:
            logger.error(f"Ошибка: {e}")
    ...
    async def set_car_auction(message: types.Message):
        ...
        try:
            ...
            if slot < 1 or slot > 15:
                await message.answer(f"❌ Слот должен быть от 1 до 15!")
                return
