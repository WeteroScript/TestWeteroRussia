...
def register_handlers(dp: Dispatcher):
    ...
    # ==========================================
    # ===== БИЗНЕС (уже есть в business.py, но оставляем для совместимости) =====
    # ==========================================
    
    # Бизнес хендлеры зарегистрированы в business.py
    ...
    async def dice_play(callback: types.CallbackQuery):
        ...
        try:
            ...
            win_chance = 0.35  # Изменено с 0.45 на 0.35
            multiplier = 1.3
    ...
    async def slots_spin(callback: types.CallbackQuery):
        ...
        try:
            ...
            # Проверяем на 2 или 3 одинаковых
            ...
            # Если выигрыш есть - добавляем, иначе списываем ставку
            if win > 0:
                user["money"] += win
                user["total_earned"] += win
